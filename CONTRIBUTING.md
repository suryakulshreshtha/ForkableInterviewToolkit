...
- Page Objects expose **actions**, not assertions. Tests assert.
- Prefer `get_by_role` / `get_by_label` over CSS or XPath.
- No `time.sleep()`. Use Playwright's auto-waiting or a web-first `expect()`.
- New test files go under `framework/tests/` with a `test_` prefix so pytest collects them.
...
- [ ] `pytest framework/tests -v` passes locally
- [ ] No secrets, no real personal data, no employer-identifying content
- [ ] Docs and code still agree with each other
- [ ] CI is green
